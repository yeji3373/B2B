...
class CartModel extends Model {
  ...
  public function combine_cart_status($sql = array()) {
    $where = NULL;
  ...
  public function combine_cart_product_info($sql = array()) {
    self::combine_cart_status($sql['cart']);
    $this->select('product.*')
        ->join('product', 'product.id = cart.prd_id', 'STRAIGHT');
    
    return $this->findAll();
  } 

  public function cartJoin() {
    $this->select("{$this->table}.idx AS cart_idx,
                  {$this->table}.order_qty, {$this->table}.prd_section, {$this->table}.onlyZeroTax,
                  {$this->table}.chkd, {$this->table}.updated_at")
        ->select("product.*")
  ...
  public function joins() {
    $this->join("buyers", "buyers.id = {$this->table}.buyer_id")
          ->join("product", "product.id = {$this->table}.prd_id")
          ...
          // ->join("(SELECT idx, product_price_idx, margin_level, price FROM supply_price) AS supply_price_compare", "supply_price_compare.product_price_idx = product_price.idx AND supply_price_compare.margin_level = {$this->table}.dis_section", "left outer")
          ...
          ->join("buyers_currency", "buyers_currency.buyer_id = {$this->table}.buyer_id", "LEFT OUTER")
